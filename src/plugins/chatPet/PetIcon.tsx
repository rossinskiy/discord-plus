...
const PUPIL_RANGE = 3.4; // px the pupil can drift from center
const EYE_LEFT = { x: 0.3125, y: 0.469 };
const EYE_RIGHT = { x: 0.6875, y: 0.469 };

const PET_SIZE = 40;
const OVERLAP_FRACTION = 0.62; // how much of the pet dips below the bar's top edge

function FloatingPet({ anchorRef }: { anchorRef: React.RefObject<HTMLElement>; }) {
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            if (!anchor || !pet) return;

            const rect = anchor.getBoundingClientRect();
            const left = rect.left + rect.width / 2 - PET_SIZE / 2;
            const top = rect.top - PET_SIZE * (1 - OVERLAP_FRACTION);

            pet.style.left = `${left}px`;
            pet.style.top = `${top}px`;

            const cx = left + PET_SIZE / 2;
            const cy = top + PET_SIZE / 2;
            const dx = mouseX - cx;
            const dy = mouseY - cy;
    ...
    const pupilStyle = (eye: { x: number; y: number; }): React.CSSProperties => ({
        ...
        left: `${eye.x * 100}%`,
        top: `${eye.y * 100}%`,
        width: Math.max(2, PET_SIZE * 0.09),
        height: Math.max(2, PET_SIZE * 0.09),
        borderRadius: "50%",
        background: "#fff",
...
export function PetIcon() {
    ...
    return (
        <>
            <span ref={anchorRef} style={{ display: "inline-block", width: 1, height: 1 }} />
            {mounted && anchorRef.current && ReactDOM.createPortal(
                <FloatingPet anchorRef={anchorRef as React.RefObject<HTMLElement>} />,
                document.body
            )}
        </>
    );
